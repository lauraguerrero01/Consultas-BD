...
SELECT
    ...
    MAX(p.precio) * 0.92 AS precio_maximo_EUR,
    MIN(p.precio) * 0.92 AS precio_minimo_EUR,
    AVG(p.precio) * 0.92 AS precio_medio_EUR,
    COUNT(p.codigo) AS total_productos
FROM
    Producto p
GROUP BY
...
SELECT
    COUNT(codigo) AS total_productos_mayor_igual_180
FROM
    Producto
WHERE
    precio * 0.92 >= 180;
...
SELECT
    f.nombre AS nombre_fabricante,
    COUNT(p.codigo) AS total_productos_mayor_igual_180
FROM
    Fabricante f
JOIN
    Producto p ON f.codigo = p.codigo_fabricante
...
FROM
    Fabricante f
LEFT JOIN
    Producto p ON f.codigo = p.codigo_fabricante AND p.precio * 0.92 >= 220
GROUP BY
    f.nombre
ORDER BY
    nombre;
